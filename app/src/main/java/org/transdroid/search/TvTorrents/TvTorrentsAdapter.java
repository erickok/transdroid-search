...
/**
 * An adapter that provides access to TvTorrents searches by parsing the raw HTML output.
 */
public class TvTorrentsAdapter implements ISearchAdapter {
	...
	private DefaultHttpClient prepareRequest(Context context) throws Exception {
		...
		String username = SettingsHelper.getSiteUser(context, TorrentSite.TvTorrents);
		String password = SettingsHelper.getSitePass(context, TorrentSite.TvTorrents);
		...
		loginPost.setEntity(new UrlEncodedFormEntity(Arrays.asList(new BasicNameValuePair[] {
				...
				new BasicNameValuePair("posted", "true"), new BasicNameValuePair("cookie", "Y") })));
		HttpResponse loginResult = httpclient.execute(loginPost);
		if (loginResult.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
			// Failed to sign in
			throw new LoginException("Login failure for TvTorrents with user " + username);
		}
	...
	public List<SearchResult> search(Context context, String query, SortOrder order, int maxResults) throws Exception {
		...
		// NOTE No sorting supported by the site
	...
	protected List<SearchResult> parseHtml(String html, int maxResults) throws Exception {
		...
		try {
			...
			final String NOTORRENTS = "Not found";
			final String RESULTS = "<h2>SEARCH - ";
			final String TORRENT = "<tr style=\"height:45;\"";
			final String HASH = "hash='";
			final String HASH_END = "';";
			final String DIGEST = "digest='";
			final String DIGEST_END = "';";
	...
	private SearchResult parseHtmlItem(String htmlItem, String digest, String hash) {
		...
		// Texts to find subsequently
		final String SHOWNAME = "<td valign=\"top\" style=\"padding:2px;\">";
		final String SHOWNAME_END = "<br><a href=\"";
		final String DETAILS_END = "\">";
		final String NAME_END = "</a>";
		final String SEEDERS = "kB/s\">";
		final String SEEDERS_END = "<br>";
